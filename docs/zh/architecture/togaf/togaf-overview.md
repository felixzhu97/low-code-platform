...
- **基础设施层（Infrastructure Layer）**：
  ...
  - **AI 生成器（apps/web/src/lib/ai-generator）**：
    - AI 客户端（AI Clients）：OpenAI Client、Claude Client、DeepSeek Client、Gemini Client 等
...
- **关键数据流场景**：
  ...
  - 数据解析数据流（CSV/XML 文件 → 解析 → 数据源服务 → 数据绑定）
  - 数据映射数据流（源数据 → 映射生成 → 映射应用 → 组件渲染）
  - 布局计算数据流（组件数据 → 布局计算 → 网格对齐 → 组件位置更新）
...
- **前端技术栈**：
  ...
  - Next.js 15：全栈框架
  - TypeScript：类型系统
  - Emotion：`@emotion/react` / `@emotion/styled`，画布与编辑器区域 CSS-in-JS
  - Tailwind CSS：设计令牌、原子类、与 Emotion 共存（shadcn/ui）
  - Radix UI：组件库
  - React DnD：拖拽库
  - Recharts：图表库
  - Zustand：状态管理
  - **共享包（packages/）**：
    - `@lowcode-platform/schema`：Schema 类型定义
    - `@lowcode-platform/component-utils`：组件树工具
    - `@lowcode-platform/utils`：通用工具函数
  - **内部库（apps/web/src/lib/）**：
    - `ai-generator`：AI 生成器（OpenAI、Claude、DeepSeek 等客户端）
  - **Local Storage（浏览器本地存储）**：临时数据和用户偏好

- **后端技术栈**（与仓库 `apps/server` 一致）：
...
plantuml -tsvg docs/zh/architecture/togaf/business-architecture.puml

# 生成应用架构图
plantuml -tpng docs/zh/architecture/togaf/application-architecture.puml
plantuml -tsvg docs/zh/architecture/togaf/application-architecture.puml

# 生成数据架构图
plantuml -tpng docs/zh/architecture/togaf/data-architecture.puml
plantuml -tsvg docs/zh/architecture/togaf/data-architecture.puml

# 生成技术架构图
plantuml -tpng docs/zh/architecture/togaf/technology-architecture.puml
plantuml -tsvg docs/zh/architecture/togaf/technology-architecture.puml
```
...
- **C4 模型**（位于 `docs/en/architecture/c4/`）：专注于软件系统的层次化视图，从系统上下文到代码级别的详细设计
...
- **新增功能**：
  ...
  - 多 AI 提供商支持：支持 OpenAI、Claude、DeepSeek、Gemini、Azure OpenAI、Groq、Mistral、Ollama、SiliconFlow 等
...
- **应用架构扩展**：
  ...
  - 应用层新增：AI 生成器适配器（AI Generator Adapter）
  - 内部库新增：AI 生成器（apps/web/src/lib/ai-generator）
    - AI 客户端（OpenAI Client、Claude Client、DeepSeek Client、Gemini Client 等）
...
- **技术架构更新**：
  - 前端新增共享包（@lowcode-platform/schema、@lowcode-platform/component-utils、@lowcode-platform/utils）
  - 前端新增内部库（apps/web/src/lib/ai-generator）
  - 外部系统新增：OpenAI API、Claude API、DeepSeek API、Gemini API 等

- **数据流新增**：
  - AI 生成组件数据流：AI 生成器组件 → AI 生成器适配器 → AI 生成器（内部库） → AI 客户端 → AI API → 组件生成器 → 组件验证器 → 模板服务 → 组件渲染器 → 画布渲染
  - AI 生成页面数据流：AI 生成器组件 → AI 生成器适配器 → AI 生成器（内部库） → AI 客户端 → AI API → 页面生成器 → 页面验证器 → 模板服务 → 组件渲染器 → 画布渲染
...
- **技术架构更新**：
  ...
  - 前端共享包（@lowcode-platform/schema、@lowcode-platform/component-utils、@lowcode-platform/utils）
  - 前端内部库（ai-generator）
  - 数据存储层新增 Local Storage（浏览器本地存储）
  - 外部系统新增 AI API 服务（OpenAI API、Claude API、DeepSeek API、Gemini API 等）
...
- [C4（中文目录）](../c4/) · [C4（English）](../../../en/architecture/c4/c4-context.puml)
