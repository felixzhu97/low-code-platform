...
- **核心业务流程**：页面设计与构建、模板管理、协作编辑、代码导出、Schema 管理、数据管理、国际化配置、用户认证、AI 生成
- **业务能力**：可视化编辑、组件管理、数据绑定、协作、代码生成、Schema 管理、模板管理、国际化、认证、图表可视化、AI 生成
...
- **表现层（Presentation Layer）**：
  ...
  - 可视化编辑器、组件面板、属性面板、模板库、数据面板
  - **组件渲染器**：基础、布局、表单、数据、图表渲染器
  - **AI 对话与生成**：AIChat 组件、`use-ai-chat.ts` Hook、AIGenerator UI
  - 表单构建器、主题编辑器、动画编辑器

- **应用层（Application Layer）**：

  - 组件管理服务、模板管理服务、数据绑定服务
  - AI 生成器适配器（AIGeneratorAdapter）
  - 应用模板用例（ApplyTemplateUseCase）
  - JSON 工具服务、历史记录服务、Schema 管理服务

- **领域层（Domain Layer）**：

  - Component、Template、Project、DataSource、DataMapping 实体
  - ComponentFactory 组件工厂

- **基础设施层（Infrastructure Layer）**：

  - Zustand 状态管理（Canvas/Component/Data/History/Theme/UI Store）
  - LocalStorage 持久化
  - **AI 生成器内部库**（`apps/web/src/lib/ai-generator`）：
...
- **核心数据实体**：Component（`children: string[]` + `parentId`）、Template、Project、DataSource、DataMapping、ThemeConfig、HistoryRecord、PageSchema
- **数据存储**：PostgreSQL（按部署）、Redis（按部署）、对象存储（按部署）、LocalStorage（浏览器本地）
- **关键数据流**：
...
- **AI 生成器内部库**（`apps/web/src/lib/ai-generator`）：多模型客户端工厂、带重试和超时的 BaseAIClient、JSON 解析器、组件/页面生成器
- **后端技术栈**：FastAPI + Python 3 + Uvicorn + Pydantic（按部署启用）
- **数据存储**：PostgreSQL、Redis、对象存储、LocalStorage（按部署选择）
- **开发模式**：前端独立运行，无需后端；AI 直连浏览器（Ollama localhost 或云 API）；Zustand 状态存 LocalStorage
